...
static EKF *create_ekf(int x_size, int z_size, Matrix *x, Matrix *P)
{
...
// 释放内存
static void destroy_ekf(EKF *ekf)
{
    if (ekf != NULL)
    {
        M_free(ekf->I);
...
static void predict_kf(EKF *ekf, Matrix *F, Matrix *Q)
{
...
static void update_kf(EKF *ekf, Matrix *z, Matrix *H, Matrix *R)
{
    Matrix *re1 = M_T(H);
...
static void predict_ekf(EKF *ekf, Matrix *F, Matrix *Q, Matrix *(*f)(Matrix *))
{
    ekf->x = f(ekf->x);
    ekf->P = M_add_sub(1, M_mul(M_mul(F, ekf->P), M_T(F)), -1, Q);
}
static void update_ekf(EKF *ekf, Matrix *z, Matrix *H, Matrix *R, Matrix *(*h)(Matrix *, Matrix *))
{
    ekf->K = M_mul(M_mul(ekf->P, M_T(H)), M_Inverse(M_add_sub(1, M_mul(M_mul(H, ekf->P), M_T(H)), -1, R)));
