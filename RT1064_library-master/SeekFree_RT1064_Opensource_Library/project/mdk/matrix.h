...
static Matrix *Matrix_gen(int row, int column, MATRIX_TYPE *data);

static Matrix *Matrix_copy(Matrix *_mat_sourse);

static Matrix *M_mul(Matrix *_mat_left, Matrix *_mat_right);

static Matrix *M_add_sub(MATRIX_TYPE scale_mat_subed, Matrix *_mat_subed, MATRIX_TYPE scale_mat_minus, Matrix *_mat_minus);

static int M_print(Matrix *_mat);

static Matrix *M_I(int order);

static int M_E_trans(Matrix *_mat, Etrans_struct *_Etrans_, int line_setting);

static Matrix *Etrans_2_Matrix(Etrans_struct *_Etrans_, int order, int line_setting);

static Matrix *Etrans_4_Inverse(Matrix *_mat_result, Etrans_struct *_Etrans_, int line_setting);

static Uptri_struct *M_Uptri_(Matrix *_mat_source);

static M_inv_struct *M_Uptri_4inv(Matrix *_mat_source);

static Lowtri_struct *M_Lowtri_(Matrix *_mat_source);

static M_inv_struct *M_Lowtri_4inv(Matrix *_mat_source);

static Matrix *M_Dia_Inv(Matrix *_mat_source);

static Dia_struct *M_Diatri_(Matrix *_mat_source);

static Matrix *M_Inverse(Matrix *_mat);

static int M_Swap(Matrix *_mat, int _line_1, int _line_2, int line_setting);

static Matrix *M_Cut(Matrix *_mat, int row_head, int row_tail, int column_head, int column_tail);

static Matrix *M_T(Matrix *_mat_source);

static int M_free(Matrix *_mat);

static MATRIX_TYPE M_tr(Matrix *_mat);

static MATRIX_TYPE M_det(Matrix *_mat);

static Matrix *M_full(Matrix *_mat, int row_up, int row_down, int column_left, int column_right, MATRIX_TYPE full_data);

static MATRIX_TYPE M_norm(Matrix *_mat, int Setting);

static Matrix *M_abs(Matrix *_mat_origin);

static Matrix *M_numul(Matrix *_mat, MATRIX_TYPE _num);

static Matrix *M_matFull(Matrix *_mat, int row_up, int column_left, Matrix *_mat_full);

static Matrix *M_Zeros(int row, int column);

static Matrix *M_Ones(int row, int column);

static Matrix *M_find(Matrix *_mat, MATRIX_TYPE value);

static Matrix *M_sum(Matrix *_mat);

static int Min_position(MATRIX_TYPE *data, int size);

static Matrix *M_min(Matrix *_mat);

static Matrix *M_max(Matrix *_mat);

static Matrix *M_minax_val(Matrix *_mat, Matrix *_mat_position);

static Matrix *M_logic_equal(Matrix *_mat, MATRIX_TYPE value);

static Matrix *M_logic(Matrix *_mat_left, Matrix *_mat_right, int Operation);

static Matrix *M_pmuldiv(Matrix *_mat_left, Matrix *_mat_right, int operation);

static Matrix *M_setval(Matrix *_mat_ini, Matrix *_mat_val, Matrix *_mat_order, int model);

static Matrix *M_numul_m(Matrix *_mat, Matrix *_mat_multi);

static M_eigen_struct *M_eigen_max(Matrix *_mat);

static int help(char *file_name);

static int M_rank(Matrix *_mat);

static int Etrans_free(Etrans_struct *_Etrans_);

static Matrix *Hilbert(int order);

static double M_cond(Matrix *_mat, int Setting);

static Matrix ** M_eigen (Matrix *_mat);

static Matrix * householder(Matrix * _x);

static Matrix * M_householder(Matrix * _mat);

static Matrix ** M_QR(Matrix * _mat);

static Matrix * M_eigen_val(Matrix * _mat);

static void progress_bar(int count, int total);

static Matrix ** M_SVD(Matrix * _mat);

static Matrix * M_pinv(Matrix * _mat, int _model_);

static Matrix * M_Sample(Matrix* _mat_source, Matrix* _mat_sample, int mode);
 ...
 * Help.
 * 可以调用�?, help("help")，查看help函数的使用方法和内容*/
    printf(">>HELP(");
    printf(file_name);
    printf(")\n");
...
static void *M_gen(Matrix*src, int row, int column, MATRIX_TYPE *data){
    src->row=row;
    src->column=column;
    src->data=data;
}
 ...
 * 填充矩阵*/
    ...
    for (i = 0; i < mat_result->row; i++) {
        for (j = 0; j < mat_result->column; j++) {
            if ((i >= row_up) && (i < (row_up + _mat->row))) {/*这里的双判断，可以优�?*/
 ...
 * FRO: FRO范数 */
    ...
    int i, j;
    if (row == _ONE_ || column == _ONE_) {/*向量的范�?*/
        switch (Setting) {
            case 1: {/*向量�?1范数*/
                for (i = 0; i < row; i++) {
                    for (j = 0; j < column; j++) {
                        /*使用abs()会提示，error C2668: “abs�?: 对重载函数的调用不明�?
						转而使用fabs().*/
 ...
 * [code]
    ...
    3.00
    [explain]
    即对�? mat_1 { 1,0,0,1 }中第0号�?3号元素，值为1�?
 */
 ...
 * Reference: [幂法-算法参考](https://max.book118.com/html/2017/0527/109650252.shtm) */
    ...
    if (_mat->column == _mat->row) {
        ...
        while (mat_z_gap_norm > deta) {
            ...
            mat_temp_1 = M_max(mat_y);//需要释放结果空�?
            temp_num = ((mat_temp_1)->data[0]);
            m_value = mat_y->data[temp_num];
            mat_temp_2 = mat_z;//需要释放结果空�?
            mat_z = M_numul(mat_y, 1 / m_value);
            mat_z_gap = M_add_sub(1, mat_z, 1, mat_temp_2);//需要释放结果空�?
 ...
 * 2. [求解-特征向量参考](https://blog.csdn.net/w_weixiaotao/article/details/111868086) */
    ...
    if (_mat->column == _mat->row) {
        ...
        MATRIX_TYPE coe; // core of elements, 对角线元�?/中心元素
        for(eigen_count=0;eigen_count<dim;eigen_count++){
            ...
            for (i = 0; i < dim-1; i++){
                ...
                coe = _mat_->data[i * dim + i];
                for (j = i; j<dim; j++){
                    _mat_->data[i * dim + j] /= coe; //让对角线元素归一�?
                }
...
//    // 此处感谢 645770225 提初的修改建�?, 具体�? [issue-11](https://github.com/Amoiensis/Matrix_hub/issues/11).
